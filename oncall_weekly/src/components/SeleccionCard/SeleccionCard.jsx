...
function SeleccionCard({ asignaciones, agregarAsignacion, nombresSwitch, nombresCore }) {
  ...
  const manejarGuardar = async () => {
    ...
    const existe = asignaciones.some(
      (a) => a.inicio === fechaInicio && a.fin === fechaFin &&
             a.switch === switchPerson && a.core === corePerson
    );

    if (existe) {
      toast.warning("Esta asignación ya existe");
      return;
    }
    ...
    try {
      ...
      toast.success("OnCall registrado correctamente");
      setSwitchPerson("");
      setCorePerson("");
      setFechaInicio("");
      setFechaFin("");
    // eslint-disable-next-line no-unused-vars
      ...
      toast.error("No se pudo registrar el OnCall");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className={styles.container}>
      <Card className={styles.card}>
        <Card.Body>
          <Card.Title className={styles.title}>Agregar OnCall</Card.Title>
            ...
            <Row className="mb-4">
              <Col md={6}>
                <Form.Group>
                  ...
                  <Form.Select value={switchPerson} onChange={(e) => setSwitchPerson(e.target.value)}>
                    <option value="">Selecciona</option>
                    {nombresSwitch.map((n, i) => <option key={i} value={n}>{n}</option>)}
                  </Form.Select>
                </Form.Group>
              </Col>
              <Col md={6}>
                <Form.Group>
                  ...
                  <Form.Select value={corePerson} onChange={(e) => setCorePerson(e.target.value)}>
                    <option value="">Selecciona</option>
                    {nombresCore.map((n, i) => <option key={i} value={n}>{n}</option>)}
                  </Form.Select>
                </Form.Group>
              </Col>
            </Row>
